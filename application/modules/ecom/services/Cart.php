...
class Ecom_Service_Cart extends Ecom_Service_Cart_Abstract
   ...
   /**
     ...
     */
    public function addItem(array $product, $qty, $update = 0)    {
        if (0 > $qty) {
            return false;
        }
        ...
        $item = new Ecom_Service_Cart_Item($product, $qty);

        if(isset($this->_items[$item->productId]) and !$update){
        	$item->qty += $this->_items[$item->productId]->qty;
        }
    ...
    public function removeItem($product)    {
        if (is_int($product) or isset($this->_items[$product])) {
            unset($this->_items[$product]);
        }

        if ($product instanceof Ecom_Service_Cart_Item) {
